...
import type { LiveChallengeDrill as LiveChallengeDrillType, DrillResult } from '@/core/types/drills';
import { Card } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { Badge } from '@/components/ui/Badge';
import { scoreLiveChallenge } from '@/core/scoring/engine';

interface LiveChallengeDrillProps {
  drill: LiveChallengeDrillType;
  onSubmit: (result: DrillResult) => void;
  onExit: () => void;
}

export function LiveChallengeDrill({ drill, onSubmit, onExit }: LiveChallengeDrillProps) {
  ...
  const handleSubmit = () => {
    ...
    const scoringResult = scoreLiveChallenge(userPrompt, drill.successCriteria);
      ...
      <Card>
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {drill.constraints.map((constraint, idx) => (
                <div key={idx} className="flex items-start gap-3 text-sm">
          ...
          {drill.exampleOutput && (
            <div className="p-4 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
              <h3 className="text-sm font-semibold mb-2">Example Output Format</h3>
              <pre className="text-xs text-[var(--color-text-secondary)] whitespace-pre-wrap">{drill.exampleOutput}</pre>
            </div>
          )}
        </div>
      </Card>
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-[var(--color-text-secondary)] mb-4">
              Design your prompt or workflow to meet all requirements and constraints.
            </p>
            <textarea
              value={userPrompt}
              onChange={(e) => setUserPrompt(e.target.value)}
              placeholder="Write your solution here..."
              className="w-full h-96 p-4 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] font-mono text-sm"
            />
          </div>
