...
import type { PromptDebugDrill as PromptDebugDrillType, DrillResult } from '@/core/types/drills';
import { Card } from '@/components/ui/Card';
import { Button } from '@/components/ui/Button';
import { Badge } from '@/components/ui/Badge';
import { scorePromptDebug } from '@/core/scoring/engine';

interface PromptDebugDrillProps {
  drill: PromptDebugDrillType;
  onSubmit: (result: DrillResult) => void;
  onExit: () => void;
}

export function PromptDebugDrill({ drill, onSubmit, onExit }: PromptDebugDrillProps) {
  ...
  const handleSubmit = () => {
    if (!flawsIdentified.trim() || !fixedPrompt.trim()) return;

    const scoringResult = scorePromptDebug(flawsIdentified, fixedPrompt, drill.flaws);
    ...
    const result: DrillResult = {
      ...
      timeSpent,
      userInput: { flaws: flawsIdentified, fix: fixedPrompt },
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <Card>
        <div className="p-6 space-y-4">
          <div>
            <h2 className="text-lg font-semibold mb-2">Task Context</h2>
            <p className="text-[var(--color-text-secondary)]">{drill.taskContext}</p>
          </div>
          ...
          <div className="p-4 bg-[var(--color-surface)] rounded-lg border border-[var(--color-border)]">
            ...
            <code className="text-sm text-[var(--color-text-secondary)] whitespace-pre-wrap">{drill.brokenPrompt}</code>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <div className="p-6 space-y-6">
          <div>
            <h2 className="text-lg font-semibold mb-2">Step 1: Identify the Flaws</h2>
            <p className="text-sm text-[var(--color-text-secondary)] mb-4">
              List all the problems you can identify in the broken prompt above.
            </p>
            <textarea
              value={flawsIdentified}
              onChange={(e) => setFlawsIdentified(e.target.value)}
              placeholder="List the flaws you identified..."
          ...
          <div>
            <h2 className="text-lg font-semibold mb-2">Step 2: Write the Fixed Prompt</h2>
            <p className="text-sm text-[var(--color-text-secondary)] mb-4">
              Rewrite the prompt to fix all the issues you identified.
            </p>
            <textarea
              value={fixedPrompt}
              onChange={(e) => setFixedPrompt(e.target.value)}
              placeholder="Write your fixed prompt here..."
      ...
      <div className="flex justify-end gap-4">
        ...
        <Button 
          ...
          disabled={!flawsIdentified.trim() || !fixedPrompt.trim()}
        >
          Submit Solution
        </Button>
      </div>
